...
export const PipelineUI = ({ darkMode = true }) => {
  ...
  const executePipeline = async () => {
    ...
    try {
      // Local pipeline validation and mock execution
      const mockResult = executePipelineLocally(nodes, edges);
      setExecutionResults(mockResult);
      showToast('Pipeline executed locally successfully!');
    } catch (error) {
      console.error('Error executing pipeline locally:', error);
      setExecutionResults({
        error: true,
  ...
  // Local pipeline execution logic
  const executePipelineLocally = (nodes, edges) => {
    // Basic DAG validation
    const nodeIds = new Set(nodes.map(node => node.id));
    const isValidDAG = edges.every(edge => 
    ...
    if (hasCycles) {
      throw new Error('Pipeline contains cycles - not a valid DAG');
    }

    // Mock execution results
    const executionResults = nodes.map(node => {
      const mockOutput = generateMockOutput(node);
      return {
        node_id: node.id,
        node_type: node.type,
        status: 'success',
        execution_time: Math.random() * 0.5 + 0.1, // Random time between 0.1-0.6s
        output: mockOutput,
        error: null
      };
    });
    ...
    return {
      is_dag: isValidDAG && !hasCycles,
      num_nodes: nodes.length,
      num_edges: edges.length,
      execution_results: executionResults,
      total_execution_time: executionResults.reduce((sum, result) => sum + result.execution_time, 0),
      status: 'success'
    };
  };

  // Simple cycle detection
  const checkForCycles = (nodes, edges) => {
    const graph = {};
  ...
  const generateMockOutput = (node) => {
    switch (node.type) {
      case 'customInput':
        return {
          type: node.data?.inputType?.toLowerCase() || 'text',
          value: node.data?.inputValue || 'Sample input value'
        };
      
      case 'customOutput':
        return {
          type: 'output',
          format: node.data?.outputFormat || node.data?.exportFormat || 'JSON',
          data: JSON.stringify({ 
      ...
      case 'llm':
        return {
          ...
          response: `This is a mock AI response for the prompt: "${node.data?.prompt || 'No prompt provided'}". In a real implementation, this would be generated by the AI model.`,
          input_tokens: Math.floor(Math.random() * 100) + 50,
          output_tokens: Math.floor(Math.random() * 200) + 100,
          note: 'This is a mock response - no actual AI model was called'
        };
      
      case 'text':
        return {
          type: 'text',
          value: node.data?.text || 'Sample text output'
        };
      
      case 'calculator':
        const operation = node.data?.operation || 'add';
        const operand1 = parseFloat(node.data?.operand1) || 10;
        const operand2 = parseFloat(node.data?.operand2) || 5;
        let result;
        switch (operation) {
          ...
          case 'divide': result = operand2 !== 0 ? operand1 / operand2 : 'Error: Division by zero'; break;
      ...
      case 'timer':
        return {
          type: 'timer',
          value: `Timer completed: ${node.data?.duration || 5} seconds`,
      ...
      case 'filter':
        return {
          type: 'filter',
          value: `Filtered data using condition: ${node.data?.condition || 'default filter'}`,
          condition: node.data?.condition || 'default filter'
        };
      
      case 'dataFormat':
        return {
          type: 'formatted_data',
          value: `Data formatted to ${node.data?.format || 'JSON'} format`,
      ...
      case 'notification':
        return {
          type: 'notification',
          value: `Notification sent: ${node.data?.message || 'Default notification'}`,
          message: node.data?.message || 'Default notification'
        };
      
      default:
        return {
          type: 'generic',
          value: `Output from ${node.type} node`,
          node_type: node.type
        };
    }
  };
